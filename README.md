...
联机对局中，本地计分板下方提供聊天入口：点击缩略消息框可查看当前一局的聊天历史并发送最多 40 个字符的文字；点击旁边的表情按钮可直接发送七个内置表情。双方最新消息会在对应计分区域旁显示 6 秒。
...
工程已配置 Android Gradle 导出，包名为 `com.local.medievaldice`，版本为 `1.2.1`，最低 Android 版本为 7.0（API 24），目标 API 为 35，仅打包 ARM64，并已启用联网与请求系统安装 APK 的权限。
